...
```powershell
./build/Test-ReleaseReadiness.ps1 -Tag v0.4.0 -RequireEmptyUnreleased
```

Replace `v0.4.0` with the intended release tag. The command must succeed on the exact `main` SHA being qualified before publication.
...
7. Enter the intended stable tag, for example `v0.4.0`.
...
Find-PSResource CopyGitHubRepo -Version 0.4.0 -Repository PSGallery
Publish-PSResource `
...
Prerelease Gallery publication is intentionally not enabled by the stable publication workflow. Tags such as `v0.4.0-beta1` are rejected rather than guessed or published as stable. If prerelease support is added later, the manifest prerelease metadata, tag convention, Gallery lookup, documentation, and workflow tests must be introduced together.
