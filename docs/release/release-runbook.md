...
| --- | --- | --- | --- |
| **Product release** | Stable tag plus immutable release identity | Creating or reusing the approved stable tag | Yes |
...
- **Planned** — version and scope work is still mutable.
...
- **Release Workflow Started** — **Publish Release** is operating on the approved SHA and tag request.
...
- **Tag Confirmed/Created** — the stable tag resolves to the approved commit. A tag-triggered run begins with an existing tag; manual dispatch creates or reuses it only after pre-publication validation.
...
1. **Exact candidate selected** — record the 40-character commit SHA and intended `vMAJOR.MINOR.PATCH` tag.
2. **Release readiness is `GO`** — use [`release-readiness.md`](release-readiness.md); do not recreate its capability or blocker decision here.
3. **Version metadata is aligned** — `ModuleVersion`, intended stable tag, dated changelog section, and release notes agree; `Unreleased` is empty at the tag boundary.
4. **Release metadata validation passes** — run `./build/Test-ReleaseReadiness.ps1 -Tag <tag> -RequireEmptyUnreleased` on the exact candidate.
5. **Quality evidence exists** — Windows, Ubuntu, and macOS Validate Project Quality evidence is successful for the candidate. **Publish Release** reruns the reusable validation and remains authoritative for release execution.
6. **Applicable live E2E evidence is recorded** — exact-candidate live evidence required by the quality/readiness model is present; an E2E harness existing is not the same as the release candidate having been live-validated.
7. **Security and non-functional blockers are dispositioned** — repository-security hardening, independent publisher-signing status, and any other release-required controls referenced by readiness must have an explicit blocker or accepted-limitation disposition.
8. **Publishing environment is ready** — the `powershell-gallery` GitHub environment exists and provides `PSGALLERY_API_KEY`; any configured environment approval is available.
9. **No immutable-version conflict exists** — the intended version is not already published in PowerShell Gallery or as a stable GitHub Release. The workflow checks again before mutation.
10. **First clean same-name publication ordering is respected** — when the repository itself is converted to a clean same-name Snapshot, perform and verify that replacement first; create `v0.1.0` only on the final clean replacement commit.

If any required precondition changes after approval, return to `Release Candidate` and repeat readiness review. Do not reuse stale approval.

## Preferred release procedure — manual workflow dispatch

Manual dispatch from `main` is the preferred maintainer-controlled path because **Publish Release** can validate the current branch SHA before creating the stable tag.

1. Open **Actions -> Publish Release -> Run workflow**.
2. Select **main**.
3. Enter the approved stable tag, for example `v0.1.0`.
4. Select `confirm_publish`.
5. Start the workflow.
6. Record the workflow run ID and captured `${{ github.sha }}` in the release evidence record.
7. Verify **Validate release context** confirms the dispatch ref is `main`, the captured SHA is still current `main`, and tag/version/changelog readiness passes.
8. Verify **Validate release commit** passes the reusable Windows, Ubuntu, and macOS Validate Project Quality jobs on the exact workflow SHA.
9. In **Publish release**, verify the workflow reruns readiness validation, builds the deterministic ZIP and Gallery package, generates the SPDX SBOM, creates required provenance/SBOM attestations, performs duplicate checks, creates or safely reuses the annotated tag only when it resolves to the approved SHA, publishes the Gallery package, and creates the GitHub Release with ZIP, checksum, and SBOM assets.
10. Do not manually create a second GitHub Release or move or recreate the stable tag around the workflow.
11. Continue to post-publication verification below.

## Alternate tag-triggered path

Pushing an exact stable `vMAJOR.MINOR.PATCH` tag also starts **Publish Release**. Use this path only when the tag has already been intentionally created on the exact approved candidate.

The workflow validates tag/version metadata and Validate Project Quality before package publication, but the tag is already a public immutable release identity at workflow start. A failed tag-triggered run may therefore require a new version and tag if the tag was wrong. Do not move a published stable tag to a different commit to repair a bad candidate.

For the initial clean same-name publication, follow [`publishing.md`](publishing.md): clean replacement first, then tag the final clean replacement commit.
...
After the workflow reports success, verify published state rather than assuming the final API calls produced the intended consumer experience.
...
If release-related documentation or site content changed:

- confirm the corresponding **Deploy Documentation Site** run passed its Jekyll build and generated-site integrity validation;
...
| Validate Project Quality failure | None for manual dispatch before tag creation | Diagnose actual failing Pester, analyzer, or package evidence; fix on a new commit; repeat readiness for the new SHA. |
...
| Package/artifact/SBOM/attestation failure | None for manual dispatch unless a tag already exists by another path | Fix the cause on a new candidate and rerun. Never publish without required integrity evidence. |
...
| Manual tag creation fails before ref exists | No stable tag created | Resolve the GitHub/API cause; rerun only if the exact approved candidate remains current and valid. |
| Tag exists but resolves to a different commit | Stable identity conflict | Stop. Never move/reuse the tag for a different approved SHA. Use a new version and tag after review. |
...
- Publish Release workflow run ID and failed step;
...
- checksum, SBOM, and attestation identities already created;
- post-publication verification completed or not completed;
- current user-facing recommendation supported by evidence;
- recovery decision authority and rationale; and
- follow-up or incident reference when further work is required.

Do not call the release `Complete` until every required channel and verification state is resolved.

## Manual fallback

Manual publication is a contingency, not the normal release procedure. Follow [`publishing.md`](publishing.md) and use the exact approved source revision plus the same local quality, release-readiness, package-validation, duplicate-version, integrity, and immutability rules.
...
GitHub Pages is deployed by `.github/workflows/deploy-documentation-site.yml` (**Deploy Documentation Site**) when site-related paths change, and it can be dispatched manually. The workflow:

1. checks out the exact workflow SHA;
2. builds Jekyll content;
3. runs `build/Test-GeneratedSite.ps1` against `_site`;
4. uploads the Pages artifact;
5. deploys through the `github-pages` environment; and
6. smoke-tests key published routes.

Because Pages uses its own concurrency and deployment state, documentation/site changes can be repaired and redeployed without changing an already-published module version.
