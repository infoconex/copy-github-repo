...
Copy GitHub Repository follows Semantic Versioning 2.0.0.
...
The release workflow first invokes the reusable cross-platform quality gate against the exact tagged commit. Validation jobs run with read-only repository permissions and check out `${{ github.sha }}` rather than mutable branch state.

Only after the Windows, Ubuntu, and macOS quality-gate jobs succeed can the publication job run. That job checks out the same tagged SHA, runs the release-readiness validator with the exact tag, rebuilds the deterministic GitHub release artifact, builds and validates the clean PowerShell Gallery package, checks for duplicate versions, and then publishes.
...
Before publication, the workflow uses an exact-version `Find-PSResource` lookup. If that Gallery version already exists, publication fails. The workflow never increments a version automatically and never attempts to overwrite an existing Gallery package.
...
Stable release assets are immutable in the normal publication workflow. Normal publication creates a GitHub release containing the versioned ZIP and its SHA-256 file and publishes the corresponding module version to PSGallery. Both channels are treated as immutable from the project release perspective.
...
Authenticode signing is optional for the initial PowerShell Gallery release and is not a `v0.1.0` blocker. If signing is added later, certificate and private-key handling will be designed separately as a protected release capability.
...
The convenience `install-release.ps1` bootstrap installs the latest stable GitHub release by default or an exact stable release when `-Version X.Y.Z` is supplied. It downloads the selected release ZIP and checksum, verifies SHA-256 before extraction, and then invokes the packaged installer. When that bootstrap is fetched from mutable `main`, the bootstrap itself is part of the trust boundary.

The separate `install-prerelease.ps1` bootstrap is intended for development and release-candidate testing before a GitHub Release exists. It resolves `main` to an exact Git commit SHA and downloads the source archive for that commit before invoking the same repository `install.ps1`. Because there is no published release artifact and matching checksum yet, the prerelease path does not provide the stable release checksum contract.

The documented pinned installation path avoids executing `main` by downloading a specific stable release ZIP and checksum directly. A matching SHA-256 proves that the ZIP matches the compared checksum; a checksum obtained from the same release channel does not independently establish publisher identity. See `docs/security/installation-security.md` for the complete trust model.
