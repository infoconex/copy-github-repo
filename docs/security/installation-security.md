...
Without parameters, the bootstrap resolves the latest stable GitHub release. A specific stable version can be requested from the same bootstrap; for example, after `v0.2.0` is published:

```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/infoconex/copy-github-repo/main/install-release.ps1))) -Version 0.2.0
```

If the selected version is already installed and intentional replacement is required, supply `-Force`:

```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/infoconex/copy-github-repo/main/install-release.ps1))) -Version 0.2.0 -Force
```
...
The initial stable version is `v0.1.0`; the following pattern pins version `v0.2.0` after that release is published:
