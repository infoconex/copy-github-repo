...
Describe 'Snapshot release checkpoint planning evidence' {
    BeforeEach {
        InModuleScope CopyGitHubRepo {
            ...
            $script:releaseSelection = [pscustomobject] @{
                ...
                ReleaseCount = $null
                Releases = @(
                    # Selection is deliberately publication/newest order, not ancestry order.
            ...
            Mock Invoke-CgrGitHubApiReadRequest {
                ...
                if ($joined -match '/git/ref/tags/(v\d+)') {
                    ...
                    return [pscustomobject] @{
                        ExitCode = 0
                        Output = @(([pscustomobject] @{ object = [pscustomobject] @{ type = $type; sha = "ref-$tag" } } | ConvertTo-Json -Compress))
                        ErrorText = ''
                    }
                }
    ...
    It 'coalesces multiple selected releases with the same peeled target into one checkpoint boundary' {
        InModuleScope CopyGitHubRepo {
            ...
            Mock Invoke-CgrGitHubApiReadRequest {
                $joined = $ArgumentList -join ' '
                if ($joined -match '/git/ref/tags/(v2-[ab])') {
                    return [pscustomobject] @{ ExitCode = 0; Output = @('{"object":{"type":"commit","sha":"ref-c2"}}'); ErrorText = '' }
                }
                if ($joined -match '/commits/c2') {
                    return [pscustomobject] @{ ExitCode = 0; Output = @('tree-c2'); ErrorText = '' }
                }
                if ($joined -match '/compare/c2\.\.\.head') {
                    return [pscustomobject] @{ ExitCode = 0; Output = @('ahead'); ErrorText = '' }
                }
                throw "Unexpected read request: $joined"
            }
    ...
    It 'fails closed with an actionable error when selected release targets diverge' {
        InModuleScope CopyGitHubRepo {
            ...
            Mock Invoke-CgrGitHubApiReadRequest {
                $joined = $ArgumentList -join ' '
                if ($joined -match '/git/ref/tags/(left|right)') {
                    return [pscustomobject] @{ ExitCode = 0; Output = @('{"object":{"type":"commit","sha":"ref-sha"}}'); ErrorText = '' }
                }
                if ($joined -match '/commits/(left-sha|right-sha)') {
                    return [pscustomobject] @{ ExitCode = 0; Output = @("tree-$($Matches[1])"); ErrorText = '' }
                }
                if ($joined -match '/compare/left-sha\.\.\.right-sha') {
    ...
    It 'fails closed when current HEAD differs in state but is not descended from the final selected release' {
        InModuleScope CopyGitHubRepo {
            Mock Invoke-CgrGitHubApiReadRequest {
                $joined = $ArgumentList -join ' '
                if ($joined -match '/git/ref/tags/(v\d+)') {
                    return [pscustomobject] @{ ExitCode = 0; Output = @('{"object":{"type":"commit","sha":"ref"}}'); ErrorText = '' }
                }
                if ($joined -match '/commits/(c\d+)') {
                    return [pscustomobject] @{ ExitCode = 0; Output = @("tree-$($Matches[1])"); ErrorText = '' }
                }
    ...
    It 'allows PlanOnly planning, forwards release filters exactly once, and performs no execution mutation' {
        InModuleScope CopyGitHubRepo {
            ...
            Mock Get-CgrRepository {
                [pscustomobject] @{ FullName = 'acme/source'; DefaultBranch = 'main'; Visibility = 'private' }
            }
            Mock New-CgrMigrationPlan {
                [pscustomobject] @{
                    ...
                    PlanOnly = $true
                    WillMutateGitHub = $false
                    ReleaseSelection = [pscustomobject] @{ SelectedReleaseCount = 1 }
                    ReleaseCheckpointPlan = [pscustomobject] @{ CheckpointCount = 1 }
                }
            }
            ...
            Should -Invoke New-CgrMigrationPlan -Times 1 -Exactly -ParameterFilter {
                $ContentMode -eq 'Snapshot' -and $IncludeReleases -and $PlanOnly -and $ReleaseTag -eq 'v1.*' -and $ReleaseCount -eq 1
            }
